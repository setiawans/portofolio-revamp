<template>
  <div v-if="visible" class="fixed inset-0 bg-paper-white/95 dark:bg-black/95 z-[100] font-mono p-2 md:p-8 text-black dark:text-hacker-green overflow-hidden flex flex-col backdrop-blur-sm transition-all duration-300">
    <!-- CRT Overlay -->
    ...
    <button @click="toggle" class="absolute top-4 right-4 md:top-8 md:right-8 text-xs md:text-xl font-bold hover:text-red-500 uppercase tracking-widest border border-transparent hover:border-red-500 px-3 py-2 md:px-4 md:py-2 transition-all z-[120] active:scale-95">
      [CLOSE SYSTEM]
    </button>

    <div class="h-full flex flex-col max-w-3xl mx-auto w-full transition-all duration-500 ease-in-out relative z-[115]">
      <!-- Output Area - Better Mobile Typography -->
      <div ref="outputRef" class="flex-1 overflow-y-auto mb-4 text-xs md:text-sm space-y-2 font-medium scrollbar-hide font-mono tracking-wide scroll-smooth dark:text-shadow-glow leading-relaxed">
        <div v-for="(line, i) in history" :key="i" class="break-words">
          <span class="opacity-50 mr-2 md:mr-4 text-[10px] md:text-xs select-none font-bold text-gray-500 dark:text-gray-600">[{{ line.time }}]</span>
          <span v-html="line.content"></span>
        </div>
      </div>
      
      <!-- Input Area - No Zoom on Mobile (16px) -->
      <div class="flex items-center text-base md:text-xl border-t-2 border-black dark:border-hacker-green pt-3 md:pt-4 transition-colors duration-300 dark:text-shadow-glow">
        <span class="mr-2 md:mr-4 font-bold animate-pulse text-black dark:text-hacker-green text-sm md:text-xl">root@portfolio:~#</span>
        <input 
          ref="inputRef"
          v-model="command"
          @keydown.enter="execute"
          type="text" 
          class="flex-1 bg-transparent border-none outline-none text-black dark:text-hacker-green placeholder-gray-500 font-bold text-base md:text-xl"
          placeholder="Enter command..."
          autoFocus
        />
      </div>
      <div class="mt-2 text-[10px] md:text-xs opacity-50 hidden md:block">
      ...
      <!-- Mobile Hint -->
      <div class="mt-1 text-[10px] opacity-70 md:hidden">
        Type <span class="font-bold">help</span> for commands
      </div>
    </div>
  </div>
